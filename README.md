...
Image model availability is account-specific. A model being listed in documentation does not establish account access. `npm run check:services` checks model metadata; a real generation is still needed to verify inference. The app preserves its guide and 3D preview when illustration generation fails. Each generated step image gets a separate Astra consistency review before it is shown. A rejected illustration gets one targeted edit using its actual pixels, followed by another review, within a shared three-minute deadline. Review accepts equivalent safe actions and does not infer exact scale from cropped images; visible physical errors remain blocked. Failed images have an explicit retry control. Images belong to a revision; only unchanged steps can reuse an earlier illustration.
...
Fast mode can replace an in-progress request without using another daily allowance. It reuses a finished draft when available, otherwise prioritizes a concise complete guide with lower reasoning effort. Sources and the 3D preview continue after publication, followed by illustrations. The replacement invalidates old writes and preserves progress. Provider latency and the complexity of the build still affect completion time.
